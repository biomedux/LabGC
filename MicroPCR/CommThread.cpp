...
BOOL CCommThread::OpenPort(CString strPortName, 
					   DWORD dwBaud, BYTE byData, BYTE byStop, BYTE byParity, int flow )
{
...
DWORD CCommThread::WriteComm(BYTE *pBuff, DWORD nToWrite)
{
	...
	if( !WriteFile( m_hComm, pBuff, nToWrite, &dwWritten, &m_osWrite))
	{
		...
		if ( status == ERROR_IO_PENDING)
		{
			...
			while (GetOverlappedResult( m_hComm, &m_osWrite, &dwWritten, FALSE)==FALSE)
			{
				m_SendStatus = false;
...
DWORD	ThreadWatchComm(CCommThread* pComm)
{
   ...
   {
		CString temp;
		temp = L"Error while creating ThreadWatchComm, " + pComm->m_sPortName;
		AfxMessageBox(temp);
		return FALSE;
   }
   ...
   while (pComm ->m_bConnected)//포트가 연결되면 무한 루프에 들어감
   {
	...
	//				AfxMessageBox("buff full"); //큐버퍼의 크기를 초과하면 경고 메시지 보냄
			...
			::PostMessage(hCommWnd, WM_COMM_READ, 0, 0 );//CChromaTermDlg로 데이터가 들어왔다는 메시지를 보냄
		}
...
void CCommThread::searchPort(vector<CString> &portList)
{
	for(int i=0; i<30; i++)
	{
		CString portName;
